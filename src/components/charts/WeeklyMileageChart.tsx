...
function ChartTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[7px] px-3 py-2 text-[12px] shadow-sm">
      <div className="text-[var(--text3)] font-inter mb-0.5">{weekLabel(payload[0].payload.week_start)}</div>
      <div className="text-[var(--text)] font-medium font-mono">{Number(payload[0].value).toFixed(1)} km</div>
    </div>
  )
}
  ...
  return (
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[10px] p-4">
      <div className="flex items-center justify-between mb-4">
        <span className="text-[13px] font-medium font-inter text-[var(--text)]">Weekly mileage</span>
        <span className="text-[11px] text-[var(--text3)] font-inter">Last 8 weeks</span>
      </div>
